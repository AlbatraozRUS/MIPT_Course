...
bool InitList (MyList *List)
{
    ...
    for (int index = 0; index< InitSize; index++)
        List->Element[index].data = POISON;
    ...
    List->Element[List->size - 1].next = 0;
...
{
    for (int index = List->head; index != List->tale; index = List->Element[index].next){
        if (List->Element[index].data == Elem)
            return index;
    }
...
bool AddMemList (MyList *List)
{
    ...
    for (size_t index = List->MaxSize - 1; index <= List->MaxSize * 2; index++){
        List->Element[index].data  = POISON;
    ...
    List->Element[List->MaxSize].next = 0;
    List->Element[List->MaxSize].prev = -1;
...
bool ListOK(MyList *List)
{
  ...
  if (List->Element[0].data != POISON) {List->ErrorCode     = EFIRSTELEM; return false;}
...
void DUMP (MyList* List)
{
    ...
    printf("Element [%d] is %4lg     ", i, List->Element[i].data);
